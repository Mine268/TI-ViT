...
class MANO(object):
    def __init__(self):
        ...
        self.vertex_num = 778
        self.face = {
            "right": self.layer["right"].faces,
            "left": self.layer["left"].faces,
        }
        ...
        if (
            torch.sum(
                torch.abs(
                    self.layer["left"].shapedirs[:, 0, :]
                    - self.layer["right"].shapedirs[:, 0, :]
                )
            )
            < 1
        ):
        ...
        self.orig_joint_regressor = self.layer[
            "right"
        ].J_regressor.numpy()  # same for the right and left hands
        ...
        self.sh_joint_regressor[self.sh_joints_name.index("Thumb_4")] = np.array(
            [1 if i == 745 else 0 for i in range(self.sh_joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Index_4")] = np.array(
            [1 if i == 317 else 0 for i in range(self.sh_joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Middle_4")] = np.array(
            [1 if i == 445 else 0 for i in range(self.sh_joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Ring_4")] = np.array(
            [1 if i == 556 else 0 for i in range(self.sh_joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
        self.sh_joint_regressor[self.sh_joints_name.index("Pinky_4")] = np.array(
            [1 if i == 673 else 0 for i in range(self.sh_joint_regressor.shape[1])],
            dtype=np.float32,
        ).reshape(1, -1)
