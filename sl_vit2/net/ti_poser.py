...
class TI_MANOPoser(nn.Module):
    def __init__(
        ...
        self.query_tokens = nn.Parameter(
            torch.randn(size=(self.num_joints + 1 + 1, self.embed_dim))
        )
        # layers
        self.joint_trans = nn.Sequential(
            *[
                TransformerBlock(dim=self.embed_dim, num_heads=12)
                for _ in range(num_proj_layer)
            ]
        )
        self.joint_proj = nn.Linear(
            in_features=self.embed_dim, out_features=6, bias=True
        )  # 6d rep of rotations
    ...
    def forward(
        ...
        tokens_origin = self.joint_trans(
            torch.cat(
                [self.query_tokens[None, :].expand(batch_size, -1, -1), patches_origin],
                dim=1,
            )
        )
        org_joint_tokens = tokens_origin[:, : self.num_joints]
        org_shape_token = tokens_origin[:, self.num_joints]
        org_root_token = tokens_origin[:, self.num_joints + 1]
        result["origin"] = {
            "joint": matrix_to_axis_angle(
                rotation_6d_to_matrix(self.joint_proj(org_joint_tokens))
            ),
        ...
        if cr_angle is not None:
            ...
            tokens_cr = self.joint_trans(
                torch.cat(
                    [self.query_tokens[None, :].expand(batch_size, -1, -1), patches_cr],
                    dim=1,
                )
            )
            cr_joint_tokens = tokens_cr[:, : self.num_joints]
            cr_shape_token = tokens_cr[:, self.num_joints]
            cr_root_token = tokens_cr[:, self.num_joints + 1]
            result["cr"] = {
                "joint": matrix_to_axis_angle(
                    rotation_6d_to_matrix(self.joint_proj(cr_joint_tokens))
                ),
        ...
        if hr_angle is not None:
            ...
            tokens_hr = self.joint_trans(
                torch.cat(
                    [self.query_tokens[None, :].expand(batch_size, -1, -1), patches_hr],
                    dim=1,
                )
            )
            hr_joint_tokens = tokens_hr[:, : self.num_joints]
            hr_shape_token = tokens_hr[:, self.num_joints]
            hr_root_token = tokens_hr[:, self.num_joints + 1]
            result["hr"] = {
                "joint": matrix_to_axis_angle(
                    rotation_6d_to_matrix(self.joint_proj(hr_joint_tokens))
                ),
