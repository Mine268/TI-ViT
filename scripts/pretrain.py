...
def setup(rank: int, cfg: Config, print_func: Callable=print):
    ...
    if os.path.exists(f"./checkpoints/{cfg.exp}/checkpoint.pt"):
        print_func(f"[GPU {rank}] found checkpoints,"
            f" trying reading from ./checkpoints/{cfg.exp}/checkpoint.pt")
        ckpt: Dict[str, Any] = torch.load(
            f"./checkpoints/{cfg.exp}/checkpoint.pt", map_location=device, weights_only=False)

        start_epoch = ckpt["epoch"] + 1
        model.module.load_state_dict(ckpt["model"])
        optimizer.load_state_dict(ckpt["optimizer"])

    return start_epoch, end_epoch, summary_writer, dataloader, optimizer, model
...
def train_one_epoch(
    ...
    for it, images in enumerate(dataloader):
        ...
        if log_every is not None and (it + 1) % log_every == 0:
            ...
            header = f"[GPU {rank}]     epoch={epoch} progress={it + 1}/{len(dataloader)} "
...
def main(rank: int, cfg: Config, print_func: Callable=print):
    # 1. setup
    device = torch.device(f"cuda:{rank}")
    start_epoch, end_epoch, summary_writer, dataloader, optimizer, model = \
        setup(rank, cfg, print_func)

    # 2. train
    for epoch in range(start_epoch, end_epoch + 1):
        start_time = datetime.datetime.now()
        print_func(f"[GPU {rank}] training for epoch {epoch}/{end_epoch},"
        ...
        print_func(f"[GPU {rank}] epoch {epoch} ends at {end_time.strftime('%Y-%m-%d_%H-%M-%S')},"
        ...
        if rank == 0:
            ...
            os.symlink(f"./checkpoint_{epoch}.pt",
